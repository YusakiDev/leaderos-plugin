    ...
    public static void buyItem(Player player, String productId) {
        if (RequestUtil.canRequest(player.getUniqueId())) {
            ...
            if (User.isPlayerAuthed(player)) {
                ...
                Bukkit.getFoliaLib().getScheduler().runAsync((buyItemTask) -> {
                    // Buy progress
                    try {
                        if (user == null) {
                        ...
                        if (buyRequest.getResponseCode() == HttpURLConnection.HTTP_OK) {
                            ...
                            Bukkit.getFoliaLib().getScheduler().runNextTick((updateCacheTask) -> org.bukkit.Bukkit.getPluginManager().callEvent(new UpdateCacheEvent(player.getName(), credits, UpdateType.SET)));
