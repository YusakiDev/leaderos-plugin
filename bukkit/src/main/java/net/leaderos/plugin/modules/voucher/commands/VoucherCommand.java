...
public class VoucherCommand extends BaseCommand {
    ...
    public void createCommand(Player player, Double amount) {
        ...
        try {
            ...
            Bukkit.getFoliaLib().getScheduler().runAsync((createVoucherTask) -> {
                boolean removeCredit = LeaderOSAPI.getCreditManager().remove(player.getName(), finalAmount);
                if (removeCredit) {
                    // Calls UpdateCache event for update player's cache
                    Bukkit.getFoliaLib().getScheduler().runNextTick((updateCacheTask) -> org.bukkit.Bukkit.getPluginManager().callEvent(new UpdateCacheEvent(player.getName(), finalAmount, UpdateType.REMOVE)));
