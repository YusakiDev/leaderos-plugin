...
public class CacheCommand extends BaseCommand {
    ...
    public void updateCommand(CommandSender sender, String target) {
        Bukkit.getFoliaLib().getScheduler().runAsync((updateTask) -> {
            ...
            if (amount != null) {
                Bukkit.getFoliaLib().getScheduler().runNextTick((updateEventTask) -> org.bukkit.Bukkit.getPluginManager().callEvent(new UpdateCacheEvent(target, amount, UpdateType.SET)));
