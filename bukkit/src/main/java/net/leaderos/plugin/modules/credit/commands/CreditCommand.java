...
public class CreditCommand extends BaseCommand {
    ...
    public void defaultCommand(Player player) {
        ...
        Bukkit.getFoliaLib().getScheduler().runAsync((getCreditTask) -> {
            ...
            // Update cache
            Bukkit.getFoliaLib().getScheduler().runNextTick((updateCacheTask) -> org.bukkit.Bukkit.getPluginManager().callEvent(new UpdateCacheEvent(player.getName(), amount == null ? 0 : amount, UpdateType.SET)));
    ...
    public void sendCommand(Player player, String target, Double a) {
        ...
        Bukkit.getFoliaLib().getScheduler().runAsync((sendCreditTask) -> {
            ...
            if (error == null) {
                ...
                Bukkit.getFoliaLib().getScheduler().runNextTick((updateCacheTask) -> org.bukkit.Bukkit.getPluginManager().callEvent(new UpdateCacheEvent(player.getName(), amount, UpdateType.REMOVE)));
                ...
                if (targetPlayer != null) {
                    // Calls UpdateCache event for update player's cache
                    Bukkit.getFoliaLib().getScheduler().runNextTick((updateCacheTask) -> org.bukkit.Bukkit.getPluginManager().callEvent(new UpdateCacheEvent(target, amount, UpdateType.ADD)));
    ...
    public void addCommand(CommandSender sender, String target, Double amount) {
        ...
        Bukkit.getFoliaLib().getScheduler().runAsync((addCreditTask) -> {
            boolean addCredit = LeaderOSAPI.getCreditManager().add(target, finalAmount);
            if (addCredit) {
                if (org.bukkit.Bukkit.getPlayerExact(target) != null)
                    // Calls UpdateCache event for update player's cache
                    Bukkit.getFoliaLib().getScheduler().runNextTick((updateCacheTask) -> org.bukkit.Bukkit.getPluginManager().callEvent(new UpdateCacheEvent(target, finalAmount, UpdateType.ADD)));
    ...
    public void removeCommand(CommandSender sender, String target, Double amount) {
        ...
        Double finalAmount = amount;
        Bukkit.getFoliaLib().getScheduler().runAsync((removeCreditTask) -> {
            boolean removeCredit = LeaderOSAPI.getCreditManager().remove(target, finalAmount);
            if (removeCredit) {
                if (org.bukkit.Bukkit.getPlayerExact(target) != null)
                    // Calls UpdateCache event for update player's cache
                    Bukkit.getFoliaLib().getScheduler().runNextTick((updateCacheTask) -> org.bukkit.Bukkit.getPluginManager().callEvent(new UpdateCacheEvent(target, finalAmount, UpdateType.REMOVE)));
    ...
    public void setCommand(CommandSender sender, String target, Double amount) {
        ...
        Double finalAmount = amount;
        Bukkit.getFoliaLib().getScheduler().runAsync((setCreditTask) -> {
            boolean setCredit = LeaderOSAPI.getCreditManager().set(target, finalAmount);
            if (setCredit) {
                if (org.bukkit.Bukkit.getPlayerExact(target) != null)
                    // Calls UpdateCache event for update player's cache
                    Bukkit.getFoliaLib().getScheduler().runNextTick((updateCacheTask) -> org.bukkit.Bukkit.getPluginManager().callEvent(new UpdateCacheEvent(target, finalAmount, UpdateType.SET)));
